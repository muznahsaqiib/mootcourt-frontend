...
export default function CaseCard({ item }) {
  return (
    <Link href={CASE_DETAILS_ROUTE(item.id)} className="no-underline block group h-full">
      <div className="relative flex flex-col h-full min-h-[200px] p-5 rounded-2xl
        ...
        {/* Title */}
        <h3 className="text-base font-semibold text-stone-800 leading-snug tracking-tight mb-2
          line-clamp-2 group-hover:text-rose-900 transition-colors duration-200">
          {item.title}
        </h3>

        {/* Summary — fills the card, pushes footer down */}
        <p className="flex-1 text-sm text-stone-600 leading-relaxed line-clamp-3">
          {item.summary || item.description || 'Explore this case to practice your legal arguments and courtroom skills.'}
        </p>
